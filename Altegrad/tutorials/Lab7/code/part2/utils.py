...
def find_communities_and_plot(G, no=0):
    ...
    plt.title(f"Reordered Adjacency Matrix - Graph {no}")
    plt.savefig(f"reordered_adjacency_matrix_{no}.png")
    plt.show()

    # Draw the graph
    plt.title(f'Generated graph {no}')
    nx.draw(G)
    plt.savefig(f"generated_graph_{no}.png")
    plt.show()
